...
extension QuickEntryViewController: quickEntryDelegate {
    ...
    fileprivate func addNFCVC () {
        if #available(iOS 11, *) {
            let storyboard: UIStoryboard = UIStoryboard(name: quickStoryboard, bundle:Util().getNSBundle() )
    ...
    func showLockScreen(_ lockTemp: LockDevice)  {
        if let vc = UIStoryboard(name: lockUnlockStoryboard, bundle: Util().getNSBundle()).instantiateViewController(withIdentifier: lockUnlockIdentifier) as? LockUnlockViewController {
